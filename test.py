...
def approximate_q_learning_worker(_):
    return play_game(TwentyFortyEight(), ApproximateQAgent(time_limit = 1.))
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        print("")
        print("We implemented a 5 algorithms to play this game, and evaluated their results over a 1000 games to find the best algorithm for 2048. For each algorithm, we calculated the average score achieved by the agent over the 1000 games and the average number of wins, i.e. games where the 2048 tile was created.")
        print("")
        print("----------------------------------------------")
        print("Results")
        print("----------------------------------------------")
        ...
        print("Average wins: 0.0, Standard Deviation: 0.0")
        print("")
        print("Expectimax agent, with depth 2 and with smoothness heuristic : ")
        ...
        print("Average wins: 0.0, Standard Deviation: 0.0")
        print("")
        print("Expectimax agent, with depth 3 and with smoothness heuristic : ")
        ...
        print("")
        print("Linear Approximate Q-Learning agent with 1 second of pretraining: ")
        print("Average score: 5123.824, Standard Deviation: 2444.2696133951686")
        print("Average wins: 0.0, Standard Deviation: 0.0")
        print("")
        print("Linear Approximate Q-Learning agent with 10 seconds of pretraining: ")
        print("Average score: 5286.268, Standard Deviation: 2446.353458807909")
        print("Average wins: 0.0, Standard Deviation: 0.0")
        print("")
        print("----------------------------------------------")
        ...
        print("")
        print("For more details on implementation time and learning discussions, please see log.txt.")
        print("----------------------------------------------")
        ...
        try:
            if sys.argv[1] == "play":
                ...
                with multiprocessing.Pool() as pool:
                    if agent == 'expectimax':
                        results = list(pool.imap_unordered(expectimax_worker, range(n)))
                    elif agent == 'expectimax_smoothness':
                        results = list(pool.imap_unordered(expectimax_with_smoothness_worker, range(n)))
                    elif agent == 'random':
                        results = list(pool.imap_unordered(random_worker, range(n)))
                    elif agent == 'greedy':
                        results = list(pool.imap_unordered(greedy_worker, range(n)))
                    elif agent == 'mcts':
                        results = list(pool.imap_unordered(mcts_worker, range(n)))
                    elif agent == 'q_learning':
                        results = list(pool.imap_unordered(approximate_q_learning_worker, range(n)))
